...
def load_chunk_checkpoint(checkpoint_path: Path, device: str = "cuda", logger: logging.Logger = None) -> Dict[str, torch.Tensor]:
    ...
    result['pipeline'] = {
        # Keep all original pipeline keys
        **pipeline_state,
    ...
    logger.debug(f"  Number of gaussians: {num_gaussians}")
    logger.debug(f"  Number of bilateral grids: {num_bil_grids}")
    
    return result

def combine_gaussian_models(chunk_params: List[Dict[str, torch.Tensor]], device: str = "cuda", logger: logging.Logger = None) -> Dict:
    ...
    for key in gauss_keys:
        combined['pipeline'][key] = torch.cat([p['pipeline'][key] for p in chunk_params], dim=0)
    
    # Handle bilateral grids separately
    ...
    for opt_name in optimizer_names:
        if opt_name in combined['optimizers']:
            ...
            param_groups = combined['optimizers'][opt_name]['param_groups']
            
            # Combine the state dictionaries
            state_dict = combined['optimizers'][opt_name]['state']
            if 0 in state_dict:  # Check if state exists
                exp_avg = torch.cat([p['optimizers'][opt_name]['state'][0]['exp_avg'] for p in chunk_params], dim=0)
                exp_avg_sq = torch.cat([p['optimizers'][opt_name]['state'][0]['exp_avg_sq'] for p in chunk_params], dim=0)
                step = chunk_params[0]['optimizers'][opt_name]['state'][0]['step']  # Keep step from first chunk
    ...
    if '_model.camera_optimizer.pose_adjustment' in combined['pipeline']:
        logger.info(f"Total number of camera poses: {combined['pipeline']['_model.camera_optimizer.pose_adjustment'].shape[0]:,}")
...
def main():
    ...
    for chunk_dir in chunk_dirs:
        ...
        params = load_chunk_checkpoint(latest_checkpoint, args.device, logger)
