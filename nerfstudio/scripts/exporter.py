...
def rotmat_to_quat(rotmat):
    ...
    q = np.zeros(rotmat.shape[:-2] + (4,))
    
    condition = tr > 0
    q[condition, 0] = np.sqrt(1 + tr[condition]) / 2
    q[condition, 1] = (rotmat[condition, 2, 1] - rotmat[condition, 1, 2]) / (4 * q[condition, 0])
    q[condition, 2] = (rotmat[condition, 0, 2] - rotmat[condition, 2, 0]) / (4 * q[condition, 0])
    q[condition, 3] = (rotmat[condition, 1, 0] - rotmat[condition, 0, 1]) / (4 * q[condition, 0])
    
    condition = (rotmat[..., 0, 0] > rotmat[..., 1, 1]) & (rotmat[..., 0, 0] > rotmat[..., 2, 2]) & ~condition
    q[condition, 1] = np.sqrt(1 + rotmat[condition, 0, 0] - rotmat[condition, 1, 1] - rotmat[condition, 2, 2]) / 2
    q[condition, 0] = (rotmat[condition, 2, 1] - rotmat[condition, 1, 2]) / (4 * q[condition, 1])
    q[condition, 2] = (rotmat[condition, 0, 1] + rotmat[condition, 1, 0]) / (4 * q[condition, 1])
    q[condition, 3] = (rotmat[condition, 0, 2] + rotmat[condition, 2, 0]) / (4 * q[condition, 1])
    
    condition = (rotmat[..., 1, 1] > rotmat[..., 2, 2]) & ~condition
    q[condition, 2] = np.sqrt(1 + rotmat[condition, 1, 1] - rotmat[condition, 0, 0] - rotmat[condition, 2, 2]) / 2
    q[condition, 0] = (rotmat[condition, 0, 2] - rotmat[condition, 2, 0]) / (4 * q[condition, 2])
    q[condition, 1] = (rotmat[condition, 0, 1] + rotmat[condition, 1, 0]) / (4 * q[condition, 2])
    q[condition, 3] = (rotmat[condition, 1, 2] + rotmat[condition, 2, 1]) / (4 * q[condition, 2])
    
    condition = ~condition
    q[condition, 3] = np.sqrt(1 + rotmat[condition, 2, 2] - rotmat[condition, 0, 0] - rotmat[condition, 1, 1]) / 2
    q[condition, 0] = (rotmat[condition, 1, 0] - rotmat[condition, 0, 1]) / (4 * q[condition, 3])
    q[condition, 1] = (rotmat[condition, 0, 2] + rotmat[condition, 2, 0]) / (4 * q[condition, 3])
    q[condition, 2] = (rotmat[condition, 1, 2] + rotmat[condition, 2, 1]) / (4 * q[condition, 3])
...
class ExportGaussianSplat(Exporter):
    ...
    """Scale of the oriented bounding box along each axis."""
    rotate_x: bool = False
    """Whether to rotate the model 90 degrees around the X axis before exporting."""
    ...
    def main(self) -> None:
        ...
        if self.rotate_x:
            CONSOLE.print("Starting X-axis rotation...")
            # Create rotation matrix (using NumPy)
            ...
            CONSOLE.print(f"Position rotation complete. New position range: {positions.min():.3f} to {positions.max():.3f}")
            
            if 'nx' in map_to_tensors:
                CONSOLE.print("Starting normal rotation...")
                normals = np.stack([map_to_tensors['nx'], map_to_tensors['ny'], map_to_tensors['nz']], axis=-1)
                normals = np.dot(normals, rotation_matrix.T)
                map_to_tensors['nx'], map_to_tensors['ny'], map_to_tensors['nz'] = normals[:, 0], normals[:, 1], normals[:, 2]
                CONSOLE.print("Normal rotation complete.")
            
            # Update positions
            map_to_tensors['x'], map_to_tensors['y'], map_to_tensors['z'] = positions[:, 0], positions[:, 1], positions[:, 2]
            CONSOLE.print("Position update complete.")

            # Apply rotation to scales and rotations (if present)
            if 'scale_0' in map_to_tensors:
                CONSOLE.print("Starting scale rotation...")
                scales = np.stack([map_to_tensors[f'scale_{i}'] for i in range(3) if f'scale_{i}' in map_to_tensors], axis=-1)
                CONSOLE.print(f"Scale shape: {scales.shape}")
                if scales.shape[1] == 3:
                    scales = np.dot(scales, rotation_matrix.T)
                    for i in range(3):
                        map_to_tensors[f'scale_{i}'] = scales[:, i]
                elif scales.shape[1] == 1:
                    CONSOLE.print("Scale is scalar, no rotation needed.")
                else:
                    CONSOLE.print(f"Warning: Unexpected scale shape {scales.shape}, skipping scale rotation.")
                CONSOLE.print("Scale rotation complete.")
            
            if 'rot_0' in map_to_tensors:
                CONSOLE.print("Starting quaternion rotation...")
                quats = np.stack([map_to_tensors[f'rot_{i}'] for i in range(4)], axis=-1)
                # Convert quaternions to rotation matrices
                rot_matrices = quat_to_rotmat(quats)
                CONSOLE.print(f"Rotation matrix shape: {rot_matrices.shape}")
                
                # Apply rotation
                rot_matrices = torch.from_numpy(rot_matrices)
                rotation_matrix = torch.tensor(rotation_matrix, dtype=rot_matrices.dtype, device=rot_matrices.device)
                rot_matrices = torch.matmul(rotation_matrix, rot_matrices)
                CONSOLE.print(f"Rotated rotation matrix shape: {rot_matrices.shape}")
                
                # Convert rotation matrices back to quaternions
                CONSOLE.print("Starting conversion of rotation matrices to quaternions...")
                rot_matrices_np = rot_matrices.cpu().numpy()
                quats = rotmat_to_quat(rot_matrices_np)
                CONSOLE.print(f"Converted quaternion shape: {quats.shape}")
    
                for i in range(4):
                    map_to_tensors[f'rot_{i}'] = quats[..., i]
                CONSOLE.print("Quaternion rotation complete.")

            # Rotate SH coefficients
            if model.config.sh_degree > 0:
                CONSOLE.print("Starting SH coefficient rotation...")
                import sphecerix
                from scipy.spatial.transform import Rotation

                Robj = Rotation.from_matrix(rotation_matrix)
                D_2 = sphecerix.tesseral_wigner_D(2, Robj)

                # Rotate DC term (l=0)
                # DC term doesn't change under rotation

                # Rotate l=1 terms
                Y_1 = np.stack([map_to_tensors[f'f_rest_{i}'] for i in range(3)], axis=-1)
                Y_1_rotated = np.dot(Y_1, rotation_matrix.T)
                for i in range(3):
                    map_to_tensors[f'f_rest_{i}'] = Y_1_rotated[..., i]

                # Rotate l=2 terms
                Y_2 = np.stack([map_to_tensors[f'f_rest_{i}'] for i in range(3, 8)], axis=-1).squeeze(axis=1)
                print("Shape of Y_2:", Y_2.shape)
                print("Shape of D_2:", D_2.shape)
                print("Shape of Y_2.T:", Y_2.T.shape)
                
                # Ensure D_2 and Y_2.T are compatible for matrix multiplication
                if D_2.shape[1] != Y_2.T.shape[0]:
                    raise ValueError(f"Incompatible shapes for matrix multiplication: D_2 {D_2.shape}, Y_2.T {Y_2.T.shape}")
                
                # Perform the rotation
                Y_2_rotated = np.dot(D_2, Y_2.T)
                print("Shape of Y_2_rotated after np.dot:", Y_2_rotated.shape)
                
                # Transpose the result
                Y_2_rotated = Y_2_rotated.T
                print("Final shape of Y_2_rotated:", Y_2_rotated.shape)
                for i in range(3, 8):
                    map_to_tensors[f'f_rest_{i}'] = Y_2_rotated[..., i-3]

                CONSOLE.print("SH coefficient rotation complete.")

            CONSOLE.print("X-axis rotation fully complete.")
